...
def build_parser():
    ...
    # 核心控制
    parser.add_argument("--mode", type=str, choices=["debug", "tune", "eval"], default="debug",
...
def main():
    ...
    if args.mode in ["debug", "tune"]:
        ...
        RAY_TMP = os.path.join('/data/shith/tmp', tmp_name) # 根据你的NAS结构修改前缀
    ...
    if args.mode == "debug":
        print("🛠️ 进入 DEBUG 模式：调用 Ray Tune 跑 5 个 Epoch，每个 Epoch 仅下降 2 次...")
    ...
    # 2. TUNE 模式：火力全开
        ...
        tuner = tune.Tuner(
            ...
            tune_config=tune.TuneConfig(
                # 🌟🌟🌟 这里是关键修改：把原来的 "C_AUUC" 和 "Y_AUUC" 改成带 Target 前缀的 🌟🌟🌟
        ...
        print(f"🏆 Tune 完毕！最佳参数: {best_result.config}")
        
        print("\n🚀 正在自动提取最佳模型进行最终全量验证...")
        eval_cfg = {**base_cfg, **best_result.config, "mode": "eval", "eval_ckpt_path": os.path.join(best_result.checkpoint.path, "model.pth")}
        train_trial(eval_cfg)

    # 3. EVAL 模式：指定路径跑测试 (该模式下无须唤起 Ray)
    elif args.mode == "eval":
        ckpt_to_load = args.eval_ckpt or f"./ckpts/{run_hierarchy}/best_model.pth"
        print(f"🔍 进入 EVAL 模式：尝试加载 {ckpt_to_load}")
        trial_cfg = {**base_cfg, **get_default_hyperparams(args.task, args.version)}

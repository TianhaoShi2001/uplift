...
(
    ...
    # python main.py --mode tune --task train_y --model TARNET_Baseline_PureV10 --version y_pure_v10_mix_all_same_alpha --exp_name exp_v10_mix_all --cuda $CUDA_B --num_per_gpu $NUM_GPU > $LOG_DIR/log_v10_mix_all.txt 2>&1
    
    # echo "▶️ [流水线 B] 正在训练 [5/6]: 经典双向对称对齐战 (Both 共享老机制参数)"
    # python main.py --mode tune --task train_y --model TARNET_Baseline_PureV10 --version y_pure_v10_mix_both_same_alpha --exp_name exp_v10_mix_both --cuda $CUDA_B --num_per_gpu $NUM_GPU > $LOG_DIR/log_v10_mix_both.txt 2>&1
    
    # echo "▶️ [流水线 B] 正在训练 [6/6]: 特定双向组合战 (Wool + Walkin 共享同一参数)"
    # python main.py --mode tune --task train_y --model TARNET_Baseline_PureV10 --version y_pure_v10_mix_wool_walkin_same_alpha --exp_name exp_v10_mix_wool_walkin --cuda $CUDA_B --num_per_gpu $NUM_GPU > $LOG_DIR/log_v10_mix_wool_walkin.txt 2>&1
